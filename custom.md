...
[custom context update] {Всегда используй $json, чтобы достать данные из предыдущих модулей. Не используй ссылки на предыдущие ноды, чтобы достать переменные. Используй ноду Merge, чтобы передать данные от нужной ноды с данными и предыдущей нодой.} (username: i_van_afanasov, datetime: 2025-06-26T23:00:26)

Всегда используй `$json`, чтобы достать данные из предыдущих модулей. Не используй ссылки на предыдущие ноды, чтобы достать переменные. Используй ноду Merge, чтобы передать данные от нужной ноды с данными и предыдущей нодой.

Нода Merge в такой конфигурации с 2мя входами:
```json
{
  "nodes": [
    {
      ...
      "id": "4cbfe9ae-dd38-43c3-956e-cd2f07a81d2f",
      "name": "Merge"
    }
  ],
  "connections": {},
